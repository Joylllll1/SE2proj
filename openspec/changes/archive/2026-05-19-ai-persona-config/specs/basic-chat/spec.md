...
- The system SHALL compose each AI request using the currently effective persona configuration
...
#### Scenario: AI reply uses session-specific persona
- **GIVEN** the current AI session has persona settings different from the user's default persona
- **WHEN** the user sends a message in that session
- **THEN** the system SHALL generate the AI reply using the session-specific persona settings
